...
struct CompileArticleResponse: Codable {
    ...
    // Injected into .rewrite and .title system prompts via ContextAssembler so output sounds
    // like THIS user (formality, register, lyricism). Independent of thinkingEvolution.
...
enum SummaryService {
    ...
    9. Enhanced note: a cleaned-up, complete version of what the user said — never a summary. Keep every fact, name, and number. Remove filler, fix grammar, complete broken thoughts, preserve the user's voice. Scale structure to length: under ~100 words → one or two clean paragraphs; longer → begin with a "**Key takeaways**" line followed by 2-4 "•" bullets (the facts someone skimming must not miss), then the full organized text with a short **bold label** line at each topic shift and "•" bullets for lists or sequences. Use ONLY **bold** inline markdown and "•" characters — no # headers, no other markdown.
    ...
    - enhancedNote: A cleaned-up, complete version of what the user said — never a summary; keep every fact, name, and number. Remove filler, fix grammar, complete broken thoughts, preserve the user's voice. Scale structure to length: under ~100 words → clean paragraphs; longer → begin with a "**Key takeaways**" line + 2-4 "•" bullets, then the full organized text with short **bold label** lines at topic shifts and "•" bullets for lists. ONLY **bold** inline markdown and "•" — no # headers, no other markdown.
    - Return ONLY JSON, no other text
    """
    }
    ...
    static func compileArticle(
        ...
        case .purpose:
            ...
            "homeLayoutJSON": "A JSON STRING (stringified — escape quotes) describing the user's home-screen layout. The outer object has {\\"sections\\": [...], \\"version\\": 1}. Each section has {\\"kindRaw\\": <one of the allowed section IDs>, \\"title\\": <optional override>, \\"rationale\\": <REQUIRED — one short sentence explaining WHY this section is here for THIS user, written in second person, max 80 chars, e.g. 'Because you ship AI apps and want to flag silent projects.'>, \\"limit\\": <optional number>, \\"staleDaysThreshold\\": <optional number>}. Pick 4-6 sections that best match this user's role. **Two universal anchors**: `knowledgeCarousel` MUST be at index 0 (the user's compiled wiki — every persona has compiled knowledge articles, this is the AI value layer of EEON), and `recentNotes` MUST be at index 1 (raw captures — every user wants to see what they captured). Persona-shaped sections start at index 2. The LLM decides whether a daily-ritual surface (todayThree, momentumPicture, lastNightsDream-equivalent) belongs at all — it's persona-dependent. Founder/builder personas should lead the persona section with `momentumPicture` (observation of focus traction, not manual intention-setting). Coaches/therapists may want `clientRoster`. Dream interpreters open on recurring imagery, not daily intentions. Do not force todayThree onto personas where it doesn't fit. Allowed kindRaw values: todayThree, priorityProjects, silentProjects, openDecisions, ideaInbox, openThreads, momentumPicture, clientRoster, followUpsPerClient, relationshipArcs, recurringPatterns, emotionalToneArc, referenceResonance, activeInquiries, contradictionLedger, knowledgeCarousel, recentNotes, dailyBrief. Knowledge + notes are hardcoded anchors — DO NOT include them in your output; they're added automatically. Pick 3-5 persona-shaped sections to follow them. FOUNDER example: momentumPicture, priorityProjects, openDecisions, ideaInbox. COACH example: clientRoster, followUpsPerClient, relationshipArcs. DREAM INTERPRETER example: recurringPatterns, emotionalToneArc, referenceResonance. RESEARCHER example: activeInquiries, contradictionLedger. **CRITICAL identity-vs-behavior rule:** The user's stated identity (PURPOSE SEED + thinkingEvolution) is non-negotiable. NEVER infer a different persona archetype from note frequency or recent capture patterns. If the user said 'founder' but their notes are people-heavy, they are a founder doing relationship work — pick founder sections, not coach sections. Note patterns inform WHICH SECTIONS within the stated persona are most relevant, NOT the persona itself. Rationales must be specific to the user's stated purpose — not generic. For todayThree, rationale should be about daily intentions in their role's language — e.g., 'Because staying on your three things is how you keep shipping.' If the user has declared focus items above, the layout MUST surface sections that make those priorities visible — e.g., if their primary focus is a specific project or theme, lift the section that shows it (priorityProjects + momentumPicture for builders; recurringPatterns for dream users; clientRoster for coaches). The user's stated focus is the strongest signal for what to lift.",
