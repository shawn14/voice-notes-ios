...
- `homeLayoutJSON` — LLM-compiled home screen layout. Still compiled and stored, but **Home no longer renders it** (2026-09-10 UX simplification: Home is a fixed Calendar → Tasks → Notes stack). The section catalog (`HomeSectionKind` in `HomeLayout.swift`) and `HomeSections.swift` stay in the codebase; re-rendering them is an engineering decision, not a tune.
...
**Google Calendar OAuth expires roughly weekly until Google approves the scope — that is not a bug in our code (verified live 2026-09-08).** Google Cloud project `eeon-9521d` is **In production** with branding verified, but Data Access for the sensitive scope `calendar.readonly` is still **under review** (Verification Center → Data access status; the Audience page's `1 user / 100 user cap` is the tell that the scope is unapproved). Grants under an unapproved sensitive scope expire on Google's ~7-day clock, so the refresh token dies and `POST oauth2.googleapis.com/token` returns `invalid_grant` / "Token has been expired or revoked." Nothing in Swift stops that clock; only verification approval does. Check status at `console.cloud.google.com/auth/verification?project=eeon-9521d` before debugging the app.

**`GoogleCalendarService.isConnected` means "a credential Google has not rejected" — never "a token blob exists."** It was the latter until 2026-09-08, and that one lying boolean took down the whole calendar: `CalendarMeetingsView.shouldReadIPhoneCalendar` returns false while Google is "connected", so a permanently-dead Google token switched OFF the EventKit fallback that exists precisely for this case — a blank calendar plus Google's raw developer error string, next to the words "Google Calendar is connected." Now: `invalid_grant` sets a persisted `needsReauthSince` (UserDefaults `googleCalendarNeedsReauthSince`), `isConnected` goes false, EventKit takes over in the SAME refresh pass, and a `needsReauth`-driven Reconnect banner shows. The banner is deliberately driven by `needsReauth` and not by `errorMessage` — `errorMessage` clears as soon as we stop calling Google, which would let a dead integration vanish silently behind iPhone Calendar results. Keep tokens written with `SecItemUpdate` (the old delete-then-add destroyed the connection whenever `SecItemAdd` failed).
...
**Order prompt invariant:** the AI-order recorder (the bottom-bar **AI Prompt** capsule, `brain.head.profile` icon, formerly paper plane) is explicit user intent for an AI-agent prompt. `AIHomeView.capturingOrder` must be reset on cancel/paywall/mic error — a leaked `true` turns the next Note into an Order (fixed 2026-09-10). It must save `Note.intentType = Order` before any async AI processing is scheduled, and `IntelligenceService` must preserve `Order`, `OrderDone`, and explicit on-device `Reminder` through extraction/reprocess. Every order capture also mirrors an encrypted durable order to the hosted connector via `AIAccessService.enqueueOrder` -> `POST /api/orders`, keyed by the note UUID, so `list_orders` can still surface the prompt when CloudKit private-memory auth has expired. Never let `SummaryService.extractIntent` overwrite an AI-order capture to `Action`/`Idea`; if `list_orders` is empty after an AI-order note, first inspect the durable queue, then the note's stored `CD_intentType`.
...
Auth: a CloudKit **management** token (Dashboard → account menu → Settings → Tokens → *Create Management Token*), saved with `xcrun cktool save-token --type management --force <token>`. Gotchas: `cktool` cannot `validate-schema` or `import-schema` against production ("endpoint not applicable"); a stale file token at `~/.config/cktool` shadows the keychain one and makes every command say "Session has expired" — move it aside. Team `BYRK5RUS4U`, container `iCloud.aivoiceeeon`. Fields register in Development only when some record syncs a non-nil value, so optional fields nobody filled on a dev device never reach Production unless added this way. EEON MCP/private-note reads are separate from schema auth: `cktool get-teams` proves the management token, but `query-records --database-type private` needs a CloudKit **user** token (`xcrun cktool save-token --type user` or `CLOUDKIT_USER_TOKEN`) or CloudKit Web Services user auth. CloudKit Web Services `ckWebAuthToken` values are single-round-trip tokens: when a response includes `ckWebAuthToken`/`webAuthToken`, discard the old token and persist the returned replacement before the next private request. Apple documents a default web-auth-token lifetime of 30 minutes after creation, or 2 weeks only if the Apple sign-in flow grants "Keep me signed in"; a hosted MCP connector cannot promise permanent unattended private-note access from a stored initial token. Hosted requests for a connected user must serialize CloudKit reads with a per-connection lock, because two overlapping requests can otherwise both spend the same single-use web-auth token and lose the rotation. Do not build a hosted connector that stores the initial web-auth token as durable; use the durable order queue for explicit agent prompts and honest reconnect for private memory.
...
- `AIHomeView.swift` — Home (2026-09-10 UX simplification): greeting → sync-failure banner → 3-row setup checklist (until done) → free-tier warning → **Calendar** (`CalendarMeetingsView`, embedded-only) → **Tasks** (3-row preview, "See All" → `TasksView`) → **Notes** (8 most recent, "See All" → `AllNotesView`) → bottom bar **Note · AI Prompt · Ask**. No lenses, tabs, filters, briefs, knowledge carousel, or persona sections on Home. Long-press Note → Type a note / Import a recording / Add a link or document (`SourcePickerSheet`)
...
**Views still in codebase but removed from navigation** (see `MEMORY.md` kill list for the why):
- 2026-09-10 UX simplification: `HomeSections.swift` (persona sections), `TodayHighlightsView.swift` + `DailyBriefSheet`, `AITabView.swift`, `LibraryView` / `LibraryCollectionView` (collections), `TagFilterSheet` / `TagManagementSheet`, `WhyThisHomeSheet`, `HomeLenses.swift` (`CalendarLensView`), `DriftDetector` home banner, `ExtractionChipsView` (note detail no longer uses it)
...
Voice-notes App Store/TestFlight builds require `EEON_CLOUDKIT_API_TOKEN` in the build environment because `voice-notes-Info.plist` uses it for native `CKFetchWebAuthTokenOperation` refresh of hosted AI access. Known path: in `~/projects/v0-eeon-app-design`, run `vercel env pull /private/tmp/eeon-vercel.env --environment=production --yes`, then either source it before Fastlane or pass `EEON_CLOUDKIT_ENV_FILE=/private/tmp/eeon-vercel.env`; the Fastlane `beta`/`release` lanes hard-stop before build-number increment if the token is unavailable.
