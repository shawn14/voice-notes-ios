...
**Order prompt invariant:** the paper-plane recorder is explicit user intent for an AI-agent prompt. It must save `Note.intentType = Order` before any async AI processing is scheduled, and `IntelligenceService` must preserve `Order`, `OrderDone`, and explicit on-device `Reminder` through extraction/reprocess. Never let `SummaryService.extractIntent` overwrite a paper-plane capture to `Action`/`Idea`; if `list_orders` is empty after a paper-plane note, first inspect the note's stored `CD_intentType`.
