...
- **Privacy-first integrations stay locked, but Google Calendar is first-class**: EEON should prefer user-controlled surfaces: Sign in with Apple, private CloudKit sync, iPhone Calendar/EventKit, direct read-only Google Calendar OAuth, Reminders sync, Share Extension ingest, and CloudKit MCP. Do not add hosted remote MCP/API, account-wide cloud vault, Gmail/Drive/task-app write-back, or broad OAuth fan-out by default; those need an explicit product/security decision because they change the privacy promise.
