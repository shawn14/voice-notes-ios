...
| `EventKitSyncService` | Pushes extracted actions to the EEON Reminders list and stores a local action->reminder map. Task toggles call `updateCompletion(for:)` so completion/reopen changes made in EEON mirror back to already-created Apple Reminders; task deletes call `deleteReminder(forActionID:)` to remove the paired reminder |
